export PegasusSquareDiag

"""
$(TYPEDSIGNATURES)
"""
struct PegasusSquareDiag <: AbstractGeometry end

"""
$(TYPEDSIGNATURES)
"""
function PegasusSquareDiag(m::Int, n::Int)
...
"""
function tensor_map(
    ::Type{PegasusSquareDiag}, ::Type{S}, nrows::Int, ncols::Int
...
$(TYPEDSIGNATURES)
"""
function gauges_list(::Type{PegasusSquareDiag}, nrows::Int, ncols::Int)
    [
        GaugeInfo(
...
$(TYPEDSIGNATURES)
"""
function MpoLayers(::Type{T}, ncols::Int) where T <: PegasusSquareDiag
    MpoLayers(
...
$(TYPEDSIGNATURES)
"""
function nodes_search_order_Mps(peps::PEPSNetwork{T, S}) where {T <: PegasusSquareDiag, S}
...
$(TYPEDSIGNATURES)
"""
function boundary(::Type{T}, ctr::MpsContractor{S}, node::Node) where {T <: PegasusSquareDiag, S}
    i, j, k = node
    if k == 1
        bnd = vcat(
...
function update_energy(
    ::Type{T}, ctr::MpsContractor{S}, σ::Vector{Int}) where {T <: PegasusSquareDiag, S}
    net = ctr.peps
    i, j, k = ctr.current_node
...
"""
function tensor(
    network::PEPSNetwork{PegasusSquareDiag, T}, node::PEPSNode, β::Real, ::Val{:sparse_pegasus_square_site}
...
$(TYPEDSIGNATURES)
"""
function projectors_site_tensor(net::PEPSNetwork{T, S}, vertex::Node) where {T <: PegasusSquareDiag, S}
    i, j = vertex
    (
...
"""
function Base.size(
    network::PEPSNetwork{PegasusSquareDiag, T}, node::PEPSNode, ::Val{:pegasus_square_site}
) where T <: AbstractSparsity
...
"""
function Base.size(
    network::PEPSNetwork{PegasusSquareDiag, T}, node::PEPSNode, ::Val{:sparse_pegasus_square_site}
) where T <: AbstractSparsity
    maximum.(projectors_site_tensor(network, Node(node)))
end
