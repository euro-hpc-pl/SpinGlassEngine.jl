...
function average_HLW_parameter_nn(st, N::Int, num_states::Int)
    ...
    for (i, k) in enumerate(1:2:num_states)
        ...
        for n in 1:N*N 
            append!(tau_l, HLW_correlation_on_site(st, n, N, k, k+1)[1])
            append!(tau_u, HLW_correlation_on_site(st, n, N, k, k+1)[2])
            append!(tau_r, HLW_correlation_on_site(st, n, N, k, k+1)[3])
            append!(tau_d, HLW_correlation_on_site(st, n, N, k, k+1)[4])
        end
...
function average_HLW_parameter_nnn(st, N::Int, num_states::Int)
    ...
    for (i, k) in enumerate(1:2:num_states)
        tau_l, tau_u, tau_r, tau_d = [], [], [], []
        for n in 1:N*N
            append!(tau_l, HLW_correlation_on_site_nnn(st, n, N, k, k+1)[1])
            append!(tau_u, HLW_correlation_on_site_nnn(st, n, N, k, k+1)[2])
            append!(tau_r, HLW_correlation_on_site_nnn(st, n, N, k, k+1)[3])
            append!(tau_d, HLW_correlation_on_site_nnn(st, n, N, k, k+1)[4])
        end
        tt[i, 1] = sum(tau_l)./length(tau_l)
        tt[i, 2] = sum(tau_u)./length(tau_u)
        tt[i, 3] = sum(tau_r)./length(tau_r)
...
function average_HLW_parameter_plaquette(st, N::Int, num_states::Int)
    ...
    for j in 1:N-1
        for n in 1:N-1
            ...
            for (i, k) in enumerate(1:2:num_states)
                count = count + 1
                t += HLW_correlations_plaquette(st, n+j*N, N, k, k+1)
            end
            append!(tau, t/count)
        end
    end
