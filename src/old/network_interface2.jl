...
export fuse_projectors, build_tensor, generate_boundary_states, generate_boundary_state_with_fusing_2
...
function generate_boundary_state(network::PegasusNetwork, v::S, w::S, state) where {S, T}
...
for x ∈ boundary_at_splitting_node(network, node)
    if length(x) == 2
        #println("a")
        v, w = x
        push!(B, generate_boundary_state(network, v, w, local_state_for_node(network, σ, v)))
    elseif length(x) == 3
        #println("b")
        v, w, k = x
...
for x ∈ boundary_at_splitting_node(network, node)
    if length(x) == 2
        v, w = x
        push(B, generate_boundary_state(network, v, w, local_state_for_node.(Ref(network), σ, Ref(v))))
    elseif length(x) == 3
        v, w, k = x
