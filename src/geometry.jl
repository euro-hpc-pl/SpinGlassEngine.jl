...
struct Pegasus{T <: AbstractTensorsLayout} <: AbstractGeometry end
...
function Pegasus{T}(m::Int, n::Int) where T
...
###    Pegasus geometry     ###
#-----------------------------
# dedicated for geometry with 2 nodes in one TN site; 
# should also work for chimera
...
function tensor_map(
    ::Type{Pegasus{T}}, ::Type{S}, nrows::Int, ncols::Int
) where {T, S <: AbstractSparsity}
    map = Dict{PEPSNode, Symbol}()

    for i ∈ 1:nrows, j ∈ 1:ncols
        push!(map, PEPSNode(i, j) => PegasusSite(S))
    end
    map
end


function gauges_list(::Type{Pegasus{T}}, nrows::Int, ncols::Int) where T
    [
        GaugeInfo(
