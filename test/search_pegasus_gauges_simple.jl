...
function bench()
    ...
    inst_filename = "$(m)x$(n)x$(t).txt"
    RESULTS_FOLDER = "$(@__DIR__)/../test/instances/square"
    ...
    for Strategy ∈ (Zipper,), Sparsity ∈ (Sparse,)
        for Gauge ∈ (GaugeStrategy,)
            for Layout ∈ (GaugesEnergy,), transform ∈ all_lattice_transformations[[1]]
                net = PEPSNetwork{SquareSingleNode{Layout},Sparsity,Float64}(m, n, potts_h, transform)
                ...
                overlaps_path = "$(@__DIR__)/overlaps$(m)x$(n)x$(t).txt"
