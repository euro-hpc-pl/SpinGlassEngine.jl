...
M, N, T = 8, 8, 4
INSTANCE_DIR = "$(@__DIR__)/../test/instances/zephyr_random/Z4/RCO/SpinGlass/single"
OUTPUT_DIR = "$(@__DIR__)/results/zephyr_random/Z4/RCO/droplets/final_bench"

if !Base.Filesystem.isdir(OUTPUT_DIR)
    Base.Filesystem.mkpath(OUTPUT_DIR)
end

BETAS = [1,] #collect(0.2:0.1:1.0)
LAYOUT = (GaugesEnergy,)
...
METHOD = :psvd_sparse
I = [1,]
eng = [40, ] #10 collect(0.01:1:1.01) 60
hamming_dist = 35 #14 74
disable_logging(LogLevel(1))
BLAS.set_num_threads(1)
cs = 2^14
iter = 2

function zephyr_sim(inst, trans, β, Layout, bd, ms, eng, hamming_dist, mstates)
    δp = 0.0
...
function run_bench(inst::String, β::Real, t, l, bd, ms, eng, hamming_dist, mstates, i)
    ...
    if isfile(out_path)
        ...
        data = try
            tic_toc = @elapsed sol, ctr, cRAM, schmidts, ldrop, droplets, ig_states = zephyr_sim(inst, t, β, l, bd, ms, eng, hamming_dist, mstates)
