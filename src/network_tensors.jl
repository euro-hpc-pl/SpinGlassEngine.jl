...
tensor_size(
    ...
    v::PEPSNode,
    ::Val{:site}
) where T <: AbstractGeometry = maximum.(projectors(network, Node(v)))
...
tensor_size(
    ...
    v::PEPSNode,
    ::Val{:sparse_site}
) where T <: AbstractGeometry = tensor_size(network, v, Val(:site))
...
function tensor(
    network::PEPSNetwork{SquareStar{T}, S},
    node::PEPSNode,
    β::Real,
    ::Val{:central_d}
...
function tensor(
    network::PEPSNetwork{SquareStar{T}, Dense},
    node::PEPSNode,
    β::Real,
    ::Val{:virtual}
