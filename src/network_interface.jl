...
boundary(network::AbstractGibbsNetwork{S,T}, node::S) where {S,T} =
    not_implemented("boundary")

node_index(network::AbstractGibbsNetwork{S,T}, node::S) where {S,T} =
    not_implemented("node_index")

update_energy(network::AbstractGibbsNetwork{S,T}, σ::Vector{Int}) where {S,T} =
    not_implemented("update_energy")

conditional_probability(network::AbstractGibbsNetwork{S,T}, v::Vector{Int}) where {S,T} =
    not_implemented("conditional_probability")

iteration_order(peps::AbstractGibbsNetwork) =
...
function _boundary_index(
    ...
    if v ∉ vertices(network.network_graph)
        return one(typeof(state))
    end
